...
class ServiceBoard extends Page
{
    protected static string|\BackedEnum|null $navigationIcon = 'heroicon-o-view-columns';

    protected string $view = 'filament.pages.service-board';

    protected static ?string $title = 'Service Board';

    protected static ?string $navigationLabel = 'Service Board';

    protected static string|\UnitEnum|null $navigationGroup = 'Managed Services';

    protected static ?int $navigationSort = 3;

    protected static ?string $slug = 'service-board/{project_id?}';
    ...
    {
        return 'Kanban board for managed service tickets';
    }
    ...
    {
        $this->projects = $this->loadAccessibleManagedProjects();

        if ($project_id !== null && $project_id !== '') {
            $projectId = (int) $project_id;

            $accessible = $this->projects->firstWhere('id', $projectId);

            if ($accessible === null) {
                Notification::make()
                    ->title('Service tidak tersedia')
                    ->body('Project yang dipilih tidak tersedia di Managed Services atau Anda tidak memiliki akses.')
                    ->warning()
                    ->send();
    ...
    /**
     * Build the list of Managed projects accessible to the current user,
     * applying membership scope for non-admin/super_admin roles.
     */
    protected function loadAccessibleManagedProjects(): Collection
    {
        $user = auth()->user();
